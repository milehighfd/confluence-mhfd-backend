...
import editProject from 'bc/routes/new-project/editproject.route.js';
...
router.post('/component-geom', async (req,res) => {
  ...
  let sql = `SELECT ST_AsGeoJSON(ST_Envelope(the_geom)) as the_geom from ${table} where objectid=${objectid}`;
  const query = {
    q: sql
  };
  logger.info(sql);
...
router.get('/get-streams-by-projectid/:projectid', [auth], async (req, res) => {
  ...
  try {
    ...
    console.log('IDS OF STREAMS', ids);
    const obj = {};
    for (const id of ids) {
      obj[id] = [];
    }
