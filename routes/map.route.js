...
router.get('/search/:query', async (req, res) => {
  ...
  let sql = `SELECT ST_x(ST_LineInterpolatePoint(st_makeline(st_linemerge(st_union(the_geom))), 0.5)) as x, ST_y(ST_LineInterpolatePoint(st_makeline(st_linemerge(st_union(the_geom))), 0.5)) as y, str_name FROM streams WHERE  str_name ILIKE '${query}%' AND ST_IsEmpty(the_geom) = false group by str_name`;
