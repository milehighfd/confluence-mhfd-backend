...
router.post('/capital', [auth, multer.array('files')], async (req, res) => {
  ...
  for (const j of splittedJurisdiction) {
    ...
    const insertQuery = `INSERT INTO ${CREATE_PROJECT_TABLE} (the_geom, jurisdiction, projectname, description, servicearea, county, status, projecttype, sponsor, overheadcost ${notRequiredFields} ,projectid, estimatedcost, component_cost, component_count, costdewatering, costmobilization, costtraffic, costutility, coststormwater, costengineering,costlegal, costconstruction, costcontingency)
      VALUES(ST_GeomFromGeoJSON('${geom}'), '${j}', '${projectname}', '${description}', '${servicearea}', '${county}', '${status}', '${projecttype}', '${sponsor}', ${isNaN(overheadcost) ? 0 : overheadcost} 
...
router.post('/study/:projectid', [auth, multer.array('files')], async (req, res) => {
  ...
  streams, jurisdiction, sponsor, cover, sendToWR} = req.body;
  const status = 'Draft';
  const projecttype = 'Study';
  const projectsubtype = 'Master Plan';
  let idsArray = JSON.parse(ids);
...
router.post('/special/:projectid', [auth, multer.array('files')], async (req, res) => {
  const user = req.user;
  const projectid = req.params.projectid;
  const {projectname, description, servicearea, county, geom, locality, jurisdiction, sponsor, cosponsor, sendToWR} = req.body;
  const status = 'Draft';
  const projecttype = 'Special';
  let notRequiredFields = '';
  if (cosponsor) {
