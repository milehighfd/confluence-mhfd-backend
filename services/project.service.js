...
const { PROJECT_STATUS, PROJECT_TYPE, PRIORITY, FILTERS } = require('../lib/enumConstants');
...
const filterProject = async (filters) => {
  ...
  for (const key in filters) {
    if (key === FILTERS.REQUEST_NAME && filters[key] != null) {
      data[key] = new RegExp(filters[key], 'i');
    } else if ((key === FILTERS.ESTIMATED_COST || key === FILTERS.MHFD_DOLLAR_REQUEST) && filters[key] != null) {
    ...
    } else if (key === FILTERS.CAPITAL_STATUS) {
